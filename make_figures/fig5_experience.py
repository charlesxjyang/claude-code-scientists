...
def main():
    ...
    # Single-line field labels (n's go in the source line at bottom)
    ax.set_xticks(pos_centers)
    ax.set_xticklabels(pretty, fontsize=10, rotation=30, ha="right",
                       rotation_mode="anchor")
    ax.set_ylabel("Years on GitHub (account creation to March 2026)")
    ...
    ax.set_title(
        "GitHub tenure (years) by field, adopters vs.\\ non-adopters",
        loc="left", fontsize=13, fontweight="bold",
    )
    n_summary = ", ".join(
        f"{pretty[i]} ({stats_table[fields[i]][0]:,}/{stats_table[fields[i]][1]})"
    ...
    add_source(
        ...
        f"Per-field non-adopter / adopter counts: {n_summary}.",
    )
