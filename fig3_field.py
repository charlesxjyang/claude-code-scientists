...
ORCID_GH_FILE = os.path.join(DATA_DIR, "orcid_github_baseline.json")  # 200k sample
...
def main():
    ...
    og_field = og["summary"]["by_field"]
    gh_rate_by_our_field = {}
    for our_field, oa_fields in FIELD_MAP.items():
        tot = sum(og_field.get(f, {}).get("orcid", 0) for f in oa_fields)
        with_g = sum(og_field.get(f, {}).get("github", 0) for f in oa_fields)
        if tot:
    ...
    add_subtitle(axA, "Among ORCID+GitHub-linked active scientists")
    ...
    add_subtitle(axB, "OpenAlex 2024–2026 sample, n = 57,969 unique authors")
    ...
    add_subtitle(axC, "P(adopt|scientist) = P(ORCID) × P(GitHub|ORCID) × P(adopt|cohort)")
    ...
    add_source(
        ...
        f"GitHub-linkage baseline n={og['summary']['global']['orcid_sampled']:,}.",
    )
