...
ROOT_DIR = os.path.dirname(SCRIPT_DIR)            # repo root (parent of make_figures/)
...
def add_source(fig, text, x=0.99, y=0.01):
    ...
    No-op by default. Set the environment variable INCLUDE_SOURCES=1
    (e.g. `INCLUDE_SOURCES=1 python3 make_figures/fig1_diffusion.py`)
    to render the source line on all figures.
    """
    if not os.environ.get("INCLUDE_SOURCES"):
        return
